...
export default function TaskItem({ task, onToggle, onDelete, onEdit, draggableProps = {} }) {
  return (
    <motion.div
      ...
      {...draggableProps}
    >
      <GripVertical className="hidden h-4 w-4 cursor-grab text-white/25 md:block" aria-hidden="true" />
      <button onClick={() => onToggle(task.id)} className={`task-toggle h-5 w-5 rounded-full border transition ${task.completed ? "is-complete" : "border-white/25"}`} aria-label={`${task.completed ? "Mark pending" : "Mark complete"}: ${task.title}`} aria-pressed={task.completed} />
      ...
      <button onClick={() => onEdit(task)} className="icon-button opacity-75 md:opacity-0 md:group-hover:opacity-100" title="Edit task" aria-label={`Edit task: ${task.title}`}><Pencil size={15} /></button>
      <button onClick={() => onDelete(task.id)} className="icon-button opacity-75 md:opacity-0 md:group-hover:opacity-100" title="Delete task" aria-label={`Delete task: ${task.title}`}><Trash2 size={15} /></button>
    </motion.div>
  );
}
